...
async function initCarte() {
  ...
  if (trace.length === 0 && balises.length === 0) {
    ...
    if (el) el.innerHTML = '<div style="height:320px;display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,.3);font-family:Exo 2,sans-serif;font-size:.9rem;">Aucune donnée GPS enregistrée</div>';
    return;
  }

  // Charger Leaflet CSS + JS si pas déjà présent
  if (!window.L) {
  ...
  // Détruire carte existante si re-init
  ...
  const tousPoints = [
    ...trace.map(p => [p.lat, p.lon]),
    ...balises.filter(b => b.lat !== '—').map(b => [parseFloat(b.lat), parseFloat(b.lon)])
  ];

  // Ajouter aussi les positions des balises DB (trouvées + non trouvées)
  if (typeof BALISES_DB !== 'undefined') {
    BALISES_DB.forEach(b => tousPoints.push([b.lat, b.lon]));
  }
  ...
  // ── Filtrer les balises DB trouvées vs non trouvées ────────────────────
  const identifiantsTrouves = new Set(balises.map(b => b.identifiant || b.qr));

  // ── Balises non trouvées (toutes les balises DB moins celles trouvées) ─
  if (typeof BALISES_DB !== 'undefined') {
    BALISES_DB.forEach(b => {
      if (identifiantsTrouves.has(b.identifiant)) return; // trouvée, gérée plus bas
      ...
      L.marker([b.lat, b.lon], { icon })
        .bindPopup(\`<b style="color:#888">${b.identifiant}</b><br/>${b.couleur} N°${b.numero}<br/><span style="color:#f44">Non trouvée</span>\`)
        .addTo(map);
    });
  }
  ...
  if (trace.length >= 2) {
    const latlngs = trace.map(p => [p.lat, p.lon]);
    L.polyline(latlngs, {
      color: '#FF6B00',
      weight: 3,
      opacity: 0.85,
    ...
    L.marker(latlngs[0], { icon: iconDepart })
      .bindPopup('<b>🟢 Départ</b>')
      .addTo(map);

    // Marqueur arrivée (jaune/or)
    const iconArrivee = L.divIcon({
      className: '',
      html: \`<div style="
        width:16px;height:16px;border-radius:50%;
        background:#FFD600;border:2px solid #fff;
        box-shadow:0 0 6px rgba(255,214,0,.7);
        box-sizing:border-box;
      "></div>\`,
      iconSize: [16, 16], iconAnchor: [8, 8]
    });
    L.marker(latlngs[latlngs.length - 1], { icon: iconArrivee })
  ...
  balises.forEach((b, idx) => {
    ...
    const couleur = b.couleur === 'Jaune' ? '#f9a825'
                  : b.couleur === 'Bleu'  ? '#42a5f5'
                  : b.couleur === 'Rouge' ? '#ef5350'
                  : '#FFD600';
    const icon = L.divIcon({
      ...
      html: \`<div style="
        ...
        box-shadow:0 0 6px ${couleur};
        display:flex;align-items:center;justify-content:center;
        font-family:Bebas Neue,sans-serif;font-size:.65rem;color:#000;
        box-sizing:border-box;
      ">${String(idx + 1).padStart(2,'0')}</div>\`,
      iconSize: [22, 22], iconAnchor: [11, 11]
    });
    const acc = b.precision ? \`<br/>±${b.precision}m\` : '';
    L.marker([parseFloat(b.lat), parseFloat(b.lon)], { icon })
      .bindPopup(\`<b style="color:${couleur}">${b.identifiant}</b><br/>Balise N°${String(b.numero).padStart(2,'0')}<br/>${formatTemps(b.tempsInter)}${acc}\`)
      .addTo(map);
  });
  ...
  if (tousPoints.length > 0) {
    try {
      map.fitBounds(L.latLngBounds(tousPoints), { padding: [20, 20] });
    } catch(_) {
      map.setView(tousPoints[0], 16);
    }
  }
}
